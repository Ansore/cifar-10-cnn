...
def cnn_train(batch, x_train, y_train, x_test, y_test):
    ...
    with tf.Session() as sess:
        ...
        last_epoch = load_model(sess, saver, 'save_model_all_con/')

        # sess.run(tf.global_variables_initializer())
        summary_writer = tf.summary.FileWriter('./tmp_all_conv', graph=tf.get_default_graph())
        ...
        for n in range(last_epoch + 1, 10000):
                ...
                if (n*num_batch+i) % 50 == 0:
                    ...
                    if acc > 0.74:
                        constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def,
                                                                                   ["outnode"])
                        with tf.gfile.FastGFile("android_all_con/model-"+str(acc)+".pb", mode='wb') as f:
            ...
            saver.save(sess, 'save_model_all_con/ifar.model', global_step=n)
        print('准确度小于0.81!')
